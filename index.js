$(document).ready(function(){
  ...
  FJS.addCallback('afterAddRecords', function(){
    var percent = (this.recordsCount - 2)*100/2;

    $('#stream_progress').text(percent + '%').attr('style', 'width: '+ percent +'%;');

    if (percent == 100){
      $('#stream_progress').parent().fadeOut(2);
    }
  });
  ...
  FJS.addCriteria({field: 'barrio', ele: '#barrio_criteria input:checkbox'});
...
function initSliders(){
  ...
  $('#barrio_criteria :checkbox').prop('checked', true);
  $('#all_barrio').on('click', function(){
    $('#barrio_criteria :checkbox').prop('checked', $(this).is(':checked'));
  });
}
